unit Controller.APICEPSeleciona.DMVC;
...
type
  [MVCPath('/api')]
  TControllerCepSelecionaApi = class(TMVCController)
  public
    [MVCPath('/($ValueApi)/cep/($Value)')]
    [MVCHTTPMethod([httpGET])]
    [MVCProduces(TMVCMediaType.APPLICATION_JSON)]
    function GetCep(const Value: String; const ValueApi : string): TJSONObject;
  end;
...
function TControllerCepSelecionaApi.GetCep(const Value: String;  const ValueApi : string): TJSONObject;
var
  Controller: iCepController;
  CepData: ICep;
begin
  try
    Controller := TCepController.New.Buscar(Value, ValueApi);
